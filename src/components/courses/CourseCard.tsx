...
// --- NEW: derive lesson count from modules -> materials ---
function getTotalLessons(course: ICourse): number {
  if (course.curriculum?.chapters?.length) {
    // BSkilling shape
    return course.curriculum.chapters.reduce(
      (acc, ch) => acc + (ch.lessons?.length || 0),
      0,
    );
  }
  // LMS shape: modules -> materials
  return (
    course.modules?.reduce(
      (acc: any, m: any) => acc + (m.materials?.length || 0),
      0,
    ) ?? 0
  );
}

// --- NEW: derive total duration in minutes from modules/materials, ---
// falling back to a flat course.duration field if present
function getTotalDurationMinutes(course: ICourse): number {
  if (course.modules?.length) {
    return course.modules.reduce((acc: any, m: any) => {
      if (m.estimatedDuration) return acc + m.estimatedDuration;
      const materialsSum =
        m.materials?.reduce(
          (mAcc: any, mat: any) => mAcc + (mat.estimatedDuration || 0),
          0,
        ) || 0;
      return acc + materialsSum;
    }, 0);
  }
  return Number(course.duration || 0);
}
...
export default function CourseCard({ course }: CourseCardProps) {
  ...
  const totalDurationMinutes = getTotalDurationMinutes(course);
  const durationLabel = formatDuration(totalDurationMinutes);
  // Fall back to createdAt if there's no explicit startTime (LMS shape doesn't have one)
  const displayDate = course.startTime || course.createdAt;
