...
// Precompute once per dataset — cheap, and keeps search O(reqs × tokens).
// ALWAYS returns an array (never undefined) so callers can't crash on it.
export function buildIndex<T extends Searchable>(items: T[]): Indexed<T>[] {
...
export function searchIndex<T extends Searchable>(
  ...
  // Guard against a null/undefined index — this was the "index is not
  // iterable" crash. Nothing to search over → nothing to return.
